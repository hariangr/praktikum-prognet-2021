...
<form action="/admindiscount/{{$discount->id}}" method="POST" class="needs-validated">
    @csrf
    @method('PUT')
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="percentage" value="{{$discount->percentage}}" placeholder="Masukan Jumlah Persentase Discount" name="percentage">
    </div>

    <label for="start">Tanggal Mulai Diskon:</label>
    <input type="date" id="start" name="start" value="{{$discount->start}}">

    <label for="start">Tanggal Akhir Diskon:</label>
    <input type="date" id="end" name="end" value="{{$discount->end}}">
